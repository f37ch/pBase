...
document.getElementById("fileform")&&document.getElementById("fileform").addEventListener("submit",e=>{
  ...
  makeRequest({file_submit:"",file:df},function(resp){
    if (resp){
      if (resp.error){
        document.getElementById("alertplace").innerHTML="<div class='alert alert-danger alert-dismissible' role='alert'   data-aos='flip-right' data-aos-delay='100'><i class='bi bi-exclamation-triangle'> </i><strong>Ошибка!</strong> "+resp.error+"<button type='button' class='btn-close' data-bs-dismiss='alert'></button></div>"
      }else{
